...
//variable I am adding
char* mime_extension;

const char* supportedMethod = "GET";
const char* supportedHttpVersion = "HTTP/1.1";
const char* defaultPHP ="index.php";
        ...
        if (connected())
        {
            ...
            if (request(&message, &length))
            {
                ...
                if (parse(line, abs_path, query))
                {
                    ...
                    //the following line had been for testing
                    //printf("type is %s\n",type);
                    if (type == NULL)
                    {
                        error(501);
...
char* indexes(const char* path)
{
    pathBuff = malloc(strlen(path)+1);
    strcpy(pathBuff,path);
    // Just for testing
    // printf("pathBuff is %s\n",path);
    // printf("pathBuff ends with %c,\n", path[strlen(pathBuff)-1]);
    if (path[strlen(pathBuff)-1]=='/')
    {
        char* path_index_php = malloc(strlen(pathBuff)+strlen(defaultPHP)+1);
        strcpy(path_index_php,pathBuff);
        strcat(path_index_php,defaultPHP);

        char* path_index_html = malloc(strlen(pathBuff)+strlen(defaultHTML)+1);
        strcpy(path_index_html,pathBuff);
        strcat(path_index_html,defaultHTML);
    
        // following lines are for test only
        // printf("path_index_php to check is %s\n",path_index_php);
        // printf("path_index_html to check is %s\n",path_index_html);
    
        // Used X_OK for php since it needs executable rights
        if (access(path_index_php, F_OK | X_OK) != -1 ) 
        {
        // Just for testing
        // printf("index.php exists.\n");
            return path_index_php;
        }
    
        else if (access(path_index_html, F_OK) != -1 ) 
        {
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // Following line was for testing only
    //printf("Entering load\n");
    
    // Following two lines I had used previously but didn't need
    //*content = NULL;
    //*length = 0;
    
    // Create initial buffer of type BYTE and null it
    BYTE* buffer = NULL;
    
    // Do initial memory allocation based on the size of BYTE
    buffer = malloc(sizeof(BYTE));
    int byte_in = 0;
    int byte_count=0;

    // Read each byte
    for (byte_in = fgetc(file); byte_in != EOF; byte_in=fgetc(file))
    {
        buffer[byte_count] = (BYTE) byte_in;
        byte_count++;
        
        // Dynanamically increase memory
        buffer = (BYTE*) realloc(buffer, sizeof(BYTE) * (byte_count+1));
    }
    
    // set content pointer to be equal to start of buffer
    *content = buffer;
    
    // set length to final count of bytes
    *length = byte_count;
...
const char* lookup(const char* path)
{
    ...
    mime_extension = strrchr(path, '.');
    
    // shift to lower case
    
    for (int i=0; i < strlen(mime_extension); i++)
    {
        mime_extension[i]=tolower(mime_extension[i]);
    }

    // The following line had been for test
    // printf("extension is %s\n", mime_extension);    
    
    
    if (strcmp(mime_extension,".html") == 0)
    {
        return "text/html";
    }
    else if (strcmp(mime_extension,".css") == 0)
    {
        return "text/css"; 
    }
    
    else if (strcmp(mime_extension,".gif") == 0)
    {
        return "image/gif"; 
    }
    
    else if (strcmp(mime_extension,".ico") == 0)
    {
        return "image/x-icon"; 
    }
    
    else if (strcmp(mime_extension,".jpg") == 0)
    {
        return "image/jpeg"; 
    }
    
    else if (strcmp(mime_extension,".js") == 0)
    {
        return "text/javascript"; 
    }
    
    else if (strcmp(mime_extension,".php") == 0)
    {
        return "text/x-php"; 
    }
    
    else if (strcmp(mime_extension,".png") == 0)
    {
        return "image/png"; 
    }
...
{
    //typical format is something like GET /favicon.ico HTTP/1.1
    ...
    // The following lines had been causing, I think, the memory issues I was having
    //free(method);
    //free(lineBuff);
    //free(httpVersion); 
    ...
    int cursor = 0;
    // Get number of characters until first space
    char* cursorBuff = strstr(lineBuff," ");
    cursor = cursorBuff - lineBuff;
    
    char method[3];
    strncpy(method,lineBuff,cursor);
    method[cursor] = 0;
    
    // Look for method
    
    // Previous ways I tried doing this ...just keeping as I struggled most with this (parsing 'GET')
    //method = malloc(cursor-1);
    //method = strncpy(method,lineBuff,cursor-1);
    //method = "GET";
    //method = strstr(line,supportedMethod);
    
    // The following line was for testing only
    // printf("method we will evaluate is %s\n",method);
    
    //if method is not GET, respond to the browser with 405 Method Not Allowed and return false;
    if (strncmp(method, supportedMethod, (strlen(method)-1))!=0)
    {
        error(405);
        return false;
    }
    ...
    // The following line was for testing only
    //printf("we will test to see if this is %c\n",lineBuff[cursor]);
    
    //check to see if request target begins with /
    if (lineBuff[cursor]!='/')
    {
    ...
    // Create counters to iterate over lineBuff
    int j=0;
    int i;

    
    for (i=cursor;lineBuff[i]!=' ' && lineBuff[i]!='?' && lineBuff[i]!='\r' && lineBuff[i]!='\n';i++)
    {
        abs_path[j]=lineBuff[i];
        j++;
        cursor = i;
    }
    ...
    cursor = cursor + 1;
    int k = 0;
        while (lineBuff[cursor]!=' ')
        {
            query[k]=lineBuff[cursor];
            k++;
            cursor++;
        }    
    }   
    ...
    httpVersion = strstr(line,supportedHttpVersion);
    
    // The following line was for testing only
    // printf("httpVersion we will evaluate is %s\n",httpVersion);
    
    if (strncmp(httpVersion, supportedHttpVersion, (strlen(httpVersion)-2))==0)
    {
        // The following line was for testing only
        // printf("THE httpVersion checks out and it is %s\n", httpVersion);
    }
    else
    {
        // If not HTTP/1.1 repose with 505 error
        error(505);
        return false;
    }
...
void transfer(const char* path, const char* type)
{
    ...
    //The follwing line had been for testing
    //printf("About to open file at path: %s\n", path);
    if (file == NULL)
    {
        error(500);
        return;
